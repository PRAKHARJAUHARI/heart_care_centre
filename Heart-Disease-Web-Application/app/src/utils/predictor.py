...
def predictor(df):
    ...
    if st.sidebar.button(label="Submit input to model"):
        joblib.dump(response_dict, "C:/Users/SSE/Heart-Disease-Web-Application/app/src/utils/payload_dump/response_dict.bin")

    if st.button(label="Predict"):
        # Load user inputs:
        response_dict = joblib.load("C:/Users/SSE/Heart-Disease-Web-Application/app/src/utils/payload_dump/response_dict.bin")
        # Append user inputs to existing dataframe:
        df = df.append(response_dict, ignore_index=True)
        # Load the saved ML model:
        model = joblib.load("C:/Users/SSE/Heart-Disease-Web-Application/app/src/models/rf_model.bin")
